 ...
 * you chose Paper + 6 because you won). In the second round, your opponent will
 * choose Paper (`B`), and you should choose Rock (`X`). This ends in a loss for
 * you with a score of 1 (1 + 0). The third round is a draw with both players
 * choosing Scissors, giving you a score of 3 + 3 = 6. In this example, if you
 * were to follow the strategy guide, you would get a total score of 15 (8 + 1 +
 * 6).
 *
 ...
 * @param {string} input
 */
export function day02(input) {
  return input
    .trim()
    .split("\n")
    .reduce((acc, line) => {
      const [opponentShape, gameResult] = line.split(" ");
      const decodedResult = gameResult.charCodeAt(0) - "X".charCodeAt(0);
      return (
        acc +
        (((decodedResult +
          ((opponentShape.charCodeAt(0) - "A".charCodeAt(0) + 2) % 3)) %
          3) +
          1 +
          decodedResult * 3)
      );
