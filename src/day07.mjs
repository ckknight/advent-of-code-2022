...
export function day07(input) {
    ...
    if (commandMatch != null) {
      ...
      if (command === "ls") {
        ...
        if (command === "cd /") {
          ...
          currentPath.push(command.slice(3).trim());
        } else {
      ...
      if (!listing) {
        throw new Error(`Unexpected line: ${line} when not listing`);
      }
      const dirMatch = /^dir (.*)$/.exec(line);
      if (dirMatch != null) {
        const [, dir] = dirMatch;
        tree[[...currentPath, dir].join("/")] = 0;
      } else {
        const fileMatch = /^(\d+) (.*)$/.exec(line);
        if (fileMatch != null) {
          const [, size, filename] = fileMatch;
          tree[[...currentPath, filename].join("/")] = Number(size);
        } else {
  ...
  const totalSize = Object.values(tree).reduce((acc, size) => acc + size, 0);

  const dirSizes = Object.entries(tree)
    .filter(([, size]) => size === 0)
    .map(([path]) => {
      const pathSlash = `${path}/`;
      return Object.entries(tree)
        .filter(([otherPath]) => otherPath.startsWith(pathSlash))
        .reduce((acc, [, size]) => acc + size, 0);
    });

  const currentUnused = 70000000 - totalSize;
  const neededToFree = 30000000 - currentUnused;
  return minBy(
    dirSizes.filter((size) => size >= neededToFree),
    (size) => size
  );
}
