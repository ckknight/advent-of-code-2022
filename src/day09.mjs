 ...
 * (which the tail also visited) and `#` marks other positions the tail
 * visited:
 *
 *     ..##..
 *     ...##.
...
/**
 * @param {string} input
 * @param {number} [tailSize]
 */
export function day09(input, tailSize = 9) {
  const visits = new Set();
  let headX = 0;
  let headY = 0;
  const tails = Array.from({ length: tailSize }, () => ({
    x: headX,
    y: headY,
  }));
  visits.add(`${headX},${headY}`);
  for (const line of input.split("\n")) {
    const { direction, distance } =
      /^(?<direction>[LRUD]) (?<distance>\d+)$/.exec(line)?.groups ?? {};
    const distanceNum = Number(distance);
    for (let index = 0; index < distanceNum; ++index) {
      switch (direction) {
        case "L":
          --headX;
          break;
      ...
      for (const tail of tails) {
        const deltaX = currentX - tail.x;
        const deltaY = currentY - tail.y;
      ...
      visits.add(`${lastTail.x},${lastTail.y}`);
    }
  }
  return visits.size;
}
