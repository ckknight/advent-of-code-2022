 ...
 * that are both driven by a precise clock circuit. The clock circuit ticks at a
 ...
 * 40 cycles after that (that is, during the 20th, 60th, 100th, 140th, 180th,
 * and 220th cycles).
 *
 ...
 * [timing](https://www.youtube.com/watch?v=sJFnWZH5FXc) the CPU instructions
 * and the CRT drawing operations, you should be able to determine whether the
 * sprite is visible the instant each pixel is drawn. If the sprite is
 * positioned such that one of its three pixels is the pixel currently being
...
/** @param {string} input */
export function day10(input, width = 40) {
  return Array.from(
    gen(input),
    (value, index) =>
      (Math.abs(value - (index % width)) <= 1 ? "#" : ".") +
      (index % width === width - 1 ? "\n" : "")
  ).join("");
}
...
/** @param {string} input */
export function day10Part1(input) {
  return Array.from(gen(input), (value, index) =>
    [20, 60, 100, 140, 180, 220].includes(index + 1) ? value * (index + 1) : 0
  ).reduce((a, b) => a + b, 0);
}
