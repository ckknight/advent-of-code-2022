 ...
 * each monkey has had one turn. The process of each monkey taking a single turn
 * is called a round.
 *
...
/** @typedef {number & { __id: true }} ID */

/** @param {string} input */
export function day11(input, rounds = 10000, decreaseWorryBy = 1) {
  const monkeys = Array.from(
    input.matchAll(
      /Monkey (\d+):\s+Starting items: (\d+(?:, \d+)*)\s+Operation: new = old ([*+]) (old|\d+)\s+Test: divisible by (\d+)\s+If true: throw to monkey (\d+)\s+If false: throw to monkey (\d+)/g
    ),
    (match, index) => {
      const [
        id,
      ...
      return {
        items: itemsCSV.split(", ").map(BigInt),
        operation: /** @type {"+" | "*"} */ (operation),
        operand:
          operand === "old" ? /** @type {const} */ ("old") : BigInt(operand),
        divisibleBy: BigInt(divisibleBy),
        whenTrue: Number(whenTrue),
        whenFalse: Number(whenFalse),
        inspections: 0,
      };
    }
  );
  ...
  for (let round = 0; round < rounds; ++round) {
    for (const monkey of monkeys) {
      const { items, operation, operand, divisibleBy, whenTrue, whenFalse } =
        monkey;
      while (true) {
        ...
        ++monkey.inspections;
        item = operate(item, operation, operand === "old" ? item : operand);
        item /= BigInt(decreaseWorryBy);
        item %= modulo;
        monkeys[item % divisibleBy ? whenFalse : whenTrue].items.push(item);
      }
    }
  }
  return monkeys
